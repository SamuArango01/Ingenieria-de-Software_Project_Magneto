...
import { generateContent, generateContentStream } from '../../../../helpers/GenerateContent';
...
Responde ÚNICAMENTE con un JSON válido con esta estructura:
{
  ...
  "topicsCovered": ["tema1", "tema2"] (lista de temas que ya se han cubierto)
     ...
     *
     * @yields {string} Text chunks from AI response
     * @returns Full accumulated response text at the end
     */
    public async *generateAIResponseStream(
        interviewId: number,
        userMessage: string
    ): AsyncGenerator<{ chunk: string; fullText: string }, AIResponseData, unknown> {
        try {
            // Get interview details
            const interview = await this.interviewRepository.findById(interviewId);
            if (!interview) {
...
TU TAREA:
1. Genera una pregunta de seguimiento natural y conversacional en español (MÁXIMO 2-3 oraciones)
2. Decide si la entrevista debe terminar basándote en:
   - ¿Se han cubierto los temas principales del tipo de entrevista?
   - ¿Las respuestas del candidato han sido consistentemente buenas o malas?
   - ¿Hay suficiente información para evaluar al candidato? (mínimo 3-4 intercambios)
